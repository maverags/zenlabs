-- ================================================================
-- SEED DATA FOR SUPABASE - SPA/SALON SYSTEM
-- ================================================================
...
-- ================================================================
-- SERVICES (Sample - 30 popular services)
-- ================================================================

INSERT INTO services (category_id, name, description, duration_minutes, base_price, member_price, display_order) 
SELECT 
    cat.id,
    service_data.name,
    ...
    service_data.price,
    service_data.member_price,
    service_data.display_order
FROM service_categories cat
CROSS JOIN LATERAL (VALUES
    -- Hair Services
    ('Hair Services', 'Women''s Haircut', 'Precision cut with style consultation', 60, 75.00, 68.00, 1),
    ('Hair Services', 'Men''s Haircut', 'Classic cut with neck cleanup', 45, 45.00, 40.00, 2),
    ('Hair Services', 'Blowout & Style', 'Professional blow-dry and styling', 45, 55.00, 50.00, 3),
    ('Hair Services', 'Full Highlights', 'Traditional foil highlights', 150, 185.00, 165.00, 4),
    ('Hair Services', 'Balayage', 'Hand-painted dimensional color', 180, 225.00, 200.00, 5),
    
    -- Spa & Massage
    ('Spa & Massage', 'Swedish Massage - 60 min', 'Relaxing full-body massage', 60, 95.00, 85.00, 1),
    ('Spa & Massage', 'Deep Tissue Massage - 60 min', 'Therapeutic muscle relief', 60, 110.00, 100.00, 2),
    ('Spa & Massage', 'Hot Stone Massage', 'Heated stones for deep relaxation', 90, 145.00, 130.00, 3),
    ('Spa & Massage', 'Couples Massage', 'Side-by-side massage for two', 60, 220.00, 200.00, 4),
    ('Spa & Massage', 'Aromatherapy Massage', 'Massage with essential oils', 75, 125.00, 110.00, 5),
    
    -- Skincare & Facials
    ('Skincare & Facials', 'Classic Facial', 'Deep cleanse, exfoliation, mask', 60, 95.00, 85.00, 1),
    ('Skincare & Facials', 'Anti-Aging Facial', 'Peptides and retinol treatment', 75, 135.00, 120.00, 2),
    ('Skincare & Facials', 'Microdermabrasion', 'Exfoliation and skin renewal', 60, 145.00, 130.00, 3),
    ('Skincare & Facials', 'Chemical Peel', 'Professional skin resurfacing', 45, 165.00, 150.00, 4),
    ('Skincare & Facials', 'Hydrating Facial', 'Intense moisture restoration', 60, 105.00, 95.00, 5),
    
    -- Nails
    ('Nails', 'Classic Manicure', 'Shape, buff, polish', 45, 35.00, 32.00, 1),
    ('Nails', 'Gel Manicure', 'Long-lasting gel polish', 60, 55.00, 50.00, 2),
    ('Nails', 'Classic Pedicure', 'Soak, scrub, polish', 60, 55.00, 50.00, 3),
    ('Nails', 'Spa Pedicure', 'Ultimate foot treatment', 90, 95.00, 85.00, 4),
    ('Nails', 'Acrylic Full Set', 'Sculptured acrylic nails', 90, 65.00, 60.00, 5),
    
    -- Body Treatments
    ('Body Treatments', 'Eyebrow Wax', 'Shape and clean brows', 15, 20.00, 18.00, 1),
    ('Body Treatments', 'Bikini Wax', 'Basic bikini line', 30, 45.00, 40.00, 2),
    ('Body Treatments', 'Brazilian Wax', 'Complete intimate area', 45, 75.00, 68.00, 3),
    ('Body Treatments', 'Full Leg Wax', 'Complete leg waxing', 45, 75.00, 68.00, 4),
    ('Body Treatments', 'Body Scrub', 'Full-body exfoliation', 60, 115.00, 105.00, 5),
    
    -- Makeup & Lashes
    ('Makeup & Lashes', 'Special Event Makeup', 'Professional makeup application', 60, 85.00, 75.00, 1),
    ('Makeup & Lashes', 'Bridal Makeup', 'Wedding day perfection', 90, 150.00, 135.00, 2),
    ('Makeup & Lashes', 'Lash Extensions - Full Set', 'Individual lash application', 120, 195.00, 175.00, 3),
    ('Makeup & Lashes', 'Lash Extensions - Fill', 'Maintenance for existing extensions', 60, 75.00, 68.00, 4),
    ('Makeup & Lashes', 'Lash Lift', 'Natural lash enhancement', 60, 85.00, 75.00, 5)
) AS service_data(category_name, name, description, duration, price, member_price, display_order)
WHERE cat.name = service_data.category_name;
...
-- ================================================================
-- STAFF AVAILABILITY (Standard schedules)
-- ================================================================
...
SELECT id, dow, '09:00'::TIME, '18:00'::TIME, '13:00'::TIME, '14:00'::TIME
FROM staff, generate_series(1, 5) dow
WHERE email = 'sarah.j@spa.com';

-- Michael Chen (Stylist) - Tue-Sat 10-7
INSERT INTO staff_availability (staff_id, day_of_week, start_time, end_time, break_start, break_end)
SELECT id, dow, '10:00'::TIME, '19:00'::TIME, '14:00'::TIME, '15:00'::TIME
FROM staff, generate_series(2, 6) dow
WHERE email = 'michael.c@spa.com';

-- Jessica Martinez (Massage) - Mon-Fri 9-5
INSERT INTO staff_availability (staff_id, day_of_week, start_time, end_time, break_start, break_end)
SELECT id, dow, '09:00'::TIME, '17:00'::TIME, '12:00'::TIME, '13:00'::TIME
FROM staff, generate_series(1, 5) dow
WHERE email = 'jessica.m@spa.com';

-- David Thompson (Massage) - Wed-Sun 11-7
INSERT INTO staff_availability (staff_id, day_of_week, start_time, end_time, break_start, break_end)
SELECT id, dow, '11:00'::TIME, '19:00'::TIME, '15:00'::TIME, '16:00'::TIME
FROM staff, UNNEST(ARRAY[0, 3, 4, 5, 6]) dow
WHERE email = 'david.t@spa.com';

-- Emily Rodriguez (Esthetician) - Mon-Fri 9-6
INSERT INTO staff_availability (staff_id, day_of_week, start_time, end_time, break_start, break_end)
SELECT id, dow, '09:00'::TIME, '18:00'::TIME, '13:00'::TIME, '14:00'::TIME
FROM staff, generate_series(1, 5) dow
WHERE email = 'emily.r@spa.com';

-- Amanda Lee (Nail Tech) - Tue-Sat 10-6
INSERT INTO staff_availability (staff_id, day_of_week, start_time, end_time, break_start, break_end)
SELECT id, dow, '10:00'::TIME, '18:00'::TIME, '13:00'::TIME, '14:00'::TIME
FROM staff, generate_series(2, 6) dow
WHERE email = 'amanda.l@spa.com';
...
-- ================================================================
-- SAMPLE CUSTOMERS (10 for testing)
-- ================================================================
...
BEGIN
    RAISE NOTICE '================================================================';
    RAISE NOTICE 'SUPABASE SEED DATA LOADED SUCCESSFULLY!';
    RAISE NOTICE '================================================================';
    RAISE NOTICE '';
    ...
    RAISE NOTICE '';
    RAISE NOTICE 'Ready for API and Agent Integration! 🚀';
    RAISE NOTICE '================================================================';
END $$;
